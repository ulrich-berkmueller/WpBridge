# MockeryWpBridge contributing guidelines

Thank you for wanting to contribute to MockeryWpBridge!

You can find below our guidelines for contribution, explaining how to send [pull requests](#pull-requests), [report issues](#filling-bugs) and [ask questions](#asking-questions), as well as which [workflow](#workflow) we're using while developing MockeryWpBridge.

## Maintainers

Current maintainers of MockeryWpBridge are:
...
If you found an error, typo, or any other flaw in the project, please report it using [GitHub Issues](https://github.com/:package_user/MockeryWpBridge/issues). Try searching the issues to see if there is an existing report of your bug, and if you'd find it, you could bump it by adding your test case there.
...
The best case would be if you'd provide a minimal reproducible test case illustrating a bug. For most cases just a code snippet would be enough, for more complex cases you can create gists or even test repos on GitHub — we would be glad to look into any problems you'll have with MockeryWpBridge.

### Asking questions

GitHub issues is not the best place for asking questions like “why my code won't work” or “is there a way to do X in MockeryWpBridge”, but we are constantly monitoring the [MockeryWpBridge tag at StackOverflow](http://stackoverflow.com/unanswered/tagged/MockeryWpBridge), so feel free to ask there! It would make it easier for other people to get answers and to keep GitHub Issues for bugs and feature requests.
...
We should note that not everything should be done as a “MockeryWpBridge feature”, some features better be a MockeryWpBridge plug-ins, some are just not in the scope of the project.

* * *

## Workflow

This section describes the workflow we use for MockeryWpBridge releases, the naming of the branches and the meaning behind them.
...
We follow [semver](http://semver.org/). We're in `0.x` at the moment, however, as MockeryWpBridge is already widely used, we don't introduce backwards-incompatible changes to our minor releases.
